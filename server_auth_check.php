...
class AuthController extends Controller
{
    ...
    public function sendOtp(Request $request, \App\Services\DialogSmsService $smsService)
    {
        ...
        // Send OTP via SMS
        $message = "Your OTP for AXIS Loyalty is: $otp. Valid for 5 minutes. Do not share this code.";
    ...
    {
        try {
            // Demo/Mock Policy Data
            $demoPolicies = [
                'AXIS-001' => 'Ahmed Raza',
                'AXIS-002' => 'Romesh Julian',
                'AXIS-003' => 'Suresh Silva',
                'POL-7777' => 'Loyalty VIP Member',
                'POL-9999' => 'Test Customer',
                'POL-1234' => 'John Doe',
                'CH-1700' => 'Premium Partner',
            ];

            // Normalize policy number for checking
            $policyNumber = strtoupper(trim($policyNumber));

            // For development/testing, return mock data from the demo list
            if (isset($demoPolicies[$policyNumber])) {
                return [
                    'valid' => true,
                    'customer_name' => $demoPolicies[$policyNumber],
